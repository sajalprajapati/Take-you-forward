...
int SearchingSinglyList(node *head,int target,int count )
{
    if(head==nullptr)
    return -1;

    count=count+1;
    int answer=(head->data==target)?(count):SearchingSinglyList(head->next,target,count);
    return answer;
}
...
int main()
{
    ...
    int count=0;
    int number= SearchingSinglyList(root,target,count);
    if(number!=-1)
